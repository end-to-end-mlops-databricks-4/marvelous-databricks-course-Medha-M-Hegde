...
class BasicModel:
    ...
    def __init__(self, config: ProjectConfig, tags: Tags, spark: SparkSession) -> None:
        ...
        self.experiment_name = "/Shared/loan-default-basic"
        self.model_name = f"{self.catalog_name}.{self.schema_name}.loan-default-basic-model"
        self.tags = tags.dict()
    ...
    def log_model_classifier(self) -> None:
        """Log the LightGBM classifier model using MLflow."""
        mlflow.set_experiment(self.experiment_name)
        with mlflow.start_run(tags=self.tags) as run:
            self.run_id = run.info.run_id
            ...
            # --- 3. Log the model using mlflow.lightgbm ---
            # `mlflow.lightgbm.log_model` is the preferred way to log LightGBM models.
            # It correctly serializes the model and its dependencies.
            # It also automatically infers the signature, so you don't need to do it manually.
            signature = infer_signature(model_input=self.X_train, model_output=y_pred_proba)

            # Log the dataset as an input artifact
            dataset = mlflow.data.from_spark(
                self.train_set_spark,
                table_name=f"{self.catalog_name}.{self.schema_name}.train_set",
                version=self.data_version,
    ...
    def register_model(self) -> None:
        ...
        registered_model = mlflow.register_model(
            model_uri=f"runs:/{self.run_id}/lightgbm-classifier-model-loan-default",
